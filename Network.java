...
public class Network {
    ...
    public User getUser(String name) {
        for(int i=0;i<userCount;i++){
            if (users[i] != null && users[i].getName() != null && users[i].getName().equals(name)) {
            return users[i];
            }
        }
        return null;
    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    ...
    public String mostPopularUser() {
        ...
        String Mostpopular ="";
        for(int i=0;i<userCount;i++){
            if(followeeCount(users[i].getName())>nummostpopular){
                nummostpopular = followeeCount(users[i].getName());
    ...
    private int followeeCount(String name) {
        int numtimes = 0;
        for(int i=0;i<userCount;i++){
            if(users[i].follows(name)){numtimes++;}
        }
        return numtimes;
    }
